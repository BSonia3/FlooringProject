 ...
 * @author sonia
 */
public class FlooringOrderDaoFileImpl implements 
        FlooringOrderDao {
    
     // load and write orders
   ...
   public void saveCurrentWork() throws FlooringPersistenceException {
       ...
       for(LocalDate date:ordersMap.keySet()){
           
           List<Order> orders = ordersMap.get(date);
   ...
   private void writeOrders(List<Order> orders,LocalDate myDate)throws FlooringPersistenceException {
    ...
    private void writeLastOrderNumber(int lastOrderNumber) throws FlooringPersistenceException {
        ...
        try {
            out = new PrintWriter(new FileWriter(ORDER_PATH + "orderNumber.txt"));
        } catch (IOException e) {
            throw new FlooringPersistenceException(
